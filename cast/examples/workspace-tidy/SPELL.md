  ...
  - id: tidy
    ...
    requirements:
      ...
      - id: preserve-unmarked
        phase: after
        description: Every pre-existing file not ending in .agentspells-disposable remains present and byte-identical after execution.
    limits: []
...
This declaration does not select an implementation. The adjacent Agent Skill is one Technique a host may bind to `tidy`. A conforming host may use a different Skill, MCP tool, script, or service as long as the declared observations, authority, and postconditions are independently evaluated.
