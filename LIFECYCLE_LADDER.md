...
### Rung 0 — Resolve which generation is FORMAT 0.3 — *absorbed into Rung 1*

**Closed as #55. The decision it carried now lives in #56.**
...
#55 established the state and pinned it as running code in `cast/tests/test_format_generations_55.py`: **FORMAT 0.3 exists in two incompatible generations**, `0.3` in the Spell domain and `0.3-candidate` in the Cast domain, and neither is a superset of the other.

It did not resolve which one survives, and **that question is not separable from Rung 1.** Adopting FORMAT 0.3 requires knowing which FORMAT 0.3 is being adopted, and the migration it implies — every declaration carrying `check` and `binding` per Requirement — is the adoption work itself rather than a prerequisite to it. Splitting them would put the decision in one ticket and its only consequence in another.

**This is kept as a struck rung rather than deleted**, because the ladder was published with six steps and a reader arriving at a five-step version would have no way to tell whether a step was completed, abandoned, or never existed.
...
**This rung now has two halves**, having absorbed the question Rung 0 carried.

**First, resolve which generation is FORMAT 0.3.** #56 proceeds under the decision to adopt the Spell generation, with the reasoning recorded so it can be defeated rather than assumed: `check` and `binding` are, in the declaration, what the Kernel independently arrived at through #13 and #32. Adopting the Cast generation would move the format backwards to meet a shape the runtime has already left behind, and it would have to be undone at Rung 2. The Cast generation is **superseded**, not deleted.

**Then take the crossing.** The consequence of the first half is that this rung is a migration of every declaration and fixture in the repository to carry `check` and `binding` per Requirement — not a schema swap.

**Entry:** none. Available now.
**Crosses:** `spell/format/SPECIFICATION.md` and `spell/format/spell.schema.json` become Archive; the 0.3 material takes their place and loses the `-draft` designation. The Cast domain's admission schema is recorded as superseded.
**Evidence:** `cast_format` emitted by the Kernel is no longer `"0.2"`; the adapter's normalization layer is gone rather than bypassed; **every migrated `check` and `binding` names a mechanism that exists, verified by casting rather than by validating**; the full suite passes.
...
A declaration that is schema-valid and names a checker nothing provides has moved the lie from prose into structure. That is the one way this rung can be taken and still leave things worse.
